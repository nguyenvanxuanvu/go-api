...
# Detect host architecture so the binary matches the Lima VM node
HOST_ARCH="$(uname -m)"
case "$HOST_ARCH" in
...
# ── Step 1: Run tests ─────────────────────────────────────────────────────────
step "[CI] Run Go tests"
cd "$REPO_ROOT"
go test ./... || fail "Tests failed — fix before deploying"
ok
...
echo "  kubectl -n ${NAMESPACE} port-forward svc/${DEPLOYMENT} 18080:80"
echo "  curl http://localhost:18080/healthz"
echo "  curl http://localhost:18080/api/v1/hello"
